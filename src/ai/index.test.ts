...
describe('ai types', () => {
  it('validates job requests and the closed task vocabulary', () => {
    expect(aiJobCreateSchema.safeParse({ task: 'coverage' }).success).toBe(
      true
    );
    expect(
    ...
    expect(
      aiJobCreateSchema.safeParse({
        task: 'coverage',
        options: { instructions: 'x'.repeat(AI_LIMITS.instructionsMax + 1) },
      }).success
    ).toBe(false);
    expect(
      suggestionAcceptSchema.safeParse({ suggestionIds: [] }).success
    ).toBe(false);
  ...
  it('every endpoint has an output schema that converts to a closed JSON Schema', () => {
    for (const e of AI_ENDPOINTS) {
      const js = z.toJSONSchema(AI_ENDPOINT_OUTPUT_SCHEMAS[e]) as {
        type: string;
  ...
  it('output schemas reject the near-misses the API validators must repair or drop', () => {
    const note = {
      ...
      title: 't',
      body: 'b',
      anchors: [{ sceneId: 'el_a', elementId: null }],
    };
    const ok = {
      logline: 'l',
      summary: 's',
    ...
    expect(
      coverageResponseSchema.safeParse({ ...ok, verdict: 'Consider' }).success
    ).toBe(false);
    expect(
      coverageResponseSchema.safeParse({
        ...ok,
        strengths: Array(AI_LIMITS.strengthsMax + 1).fill(note),
      }).success
    ).toBe(false);
    expect(
      polishResponseSchema.safeParse({
        suggestions: [{ elementId: 'el_a', after: 'x', rationale: 'r' }],
      }).success
    ).toBe(true);
    expect(
      polishResponseSchema.safeParse({ suggestions: [{ elementId: 7 }] })
        .success
    ).toBe(false);
  });

  it('registers the AI routes and error codes', () => {
    expect(API_ROUTES.aiJobCreate).toMatchObject({
      method: 'POST',
      path: '/documents/:did/ai/jobs',
    });
    expect(API_ROUTES.aiSuggestionSetAccept.path).toBe(
      '/ai/suggestion-sets/:ssid/accept'
    );
    for (const c of [
      'AI_KEY_NOT_PERMITTED',
      'AI_OUTPUT_INVALID',
      'AI_UNAVAILABLE',
      'JOB_ALREADY_RUNNING',
      'RATE_LIMITED',
