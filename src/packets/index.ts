...
/**
 * `?snapshotId=&include=` shared by the four packet routes. `include` is a comma list added to the defaults (`assets` and
 * `script` are on by default; `neighbors` and `pages` are opt-in; `-script` and `-assets` switch a default off).
 */
export const PACKET_INCLUDE_FLAGS = [
  'assets',
  'assetUrls',
  'script',
  'neighbors',
...
const pages = z
  .object({ start: z.string(), end: z.string(), eighths: z.number().int() })
  .nullable();

export const packetElementSchema = z.union([
  z.object({
    id: z.string(),
  ...
  z.object({
    ...
    type: z
      .string()
      .refine((t) => !['character', 'parenthetical', 'dialogue'].includes(t)),
    text: z.string(),
  }),
]);
...
const sceneHeader = z.object({
  ...
  folders: z.array(
    z.object({ id: z.string(), kind: z.string(), title: z.string() })
  ),
  contentHash: z.string(),
...
export const scenePacketBodySchema = z.object({
  ...
  notes: z.array(
    z.object({
      ...
      type: z.string(),
      text: z.string(),
      anchorElementId: z.string().nullable(),
    })
  ),
  sceneAssets: z.array(assetSummary),
  neighbors: z
    .object({ previous: sceneRef.nullable(), next: sceneRef.nullable() })
    .optional(),
});
...
export const characterPacketBodySchema = z.object({
  ...
  sampleDialogue: z.array(
    z.object({ elementId: z.string(), sceneId: z.string(), text: z.string() })
  ),
  relationships: z.array(
    z.object({
      entityId: z.string(),
      name: z.string(),
      sharedSceneCount: z.number().int(),
    })
  ),
  props: z.array(z.object({ entityId: z.string(), name: z.string() })),
  assets: z.array(assetSummary),
});
export const characterPacketSchema = envelope(
  'character',
  characterPacketBodySchema
);
...
export const locationPacketBodySchema = z.object({
  ...
  totals: z.object({
    sceneCount: z.number().int(),
    eighths: z.number().int(),
    timesOfDay: z.record(z.string(), z.number().int()),
  }),
  setElements: z.array(
    z.object({ entityId: z.string(), kind: z.string(), name: z.string() })
  ),
  assets: z.array(assetSummary),
});
export const locationPacketSchema = envelope(
  'location',
  locationPacketBodySchema
);
...
export type AnyPacket =
  ScenePacket | CharacterPacket | LocationPacket | ShotPacket;
